...
const HEADING_CLASSES = {
  h2: "mb-3 text-2xl leading-tight font-semibold tracking-tight first:mt-0",
  h3: "mt-6 mb-2 text-xl leading-snug font-semibold tracking-tight first:mt-0",
};
...
function renderBlock(block: Block, key: number): ReactElement {
  ...
  return (
    <p key={key} className="mt-3 text-[0.95rem] leading-relaxed first:mt-0">
      {renderInline(block.text)}
    </p>
  );
}
...
function renderInline(text: string): ReactNode[] {
  ...
  return parts.map((part, index) => {
    ...
    if (part.startsWith("`") && part.endsWith("`")) {
      return (
        <code key={index} className="rounded bg-black/10 px-1.5 py-0.5 font-mono text-[0.85em]">
          {part.slice(1, -1)}
        </code>
      );
    }
