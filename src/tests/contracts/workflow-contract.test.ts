...
} from "../../contracts/workflow/workflow-contract.js";
...
describe("workflow response contract", () => {
  ...
  it("rejects probabilities outside 0 to 1", () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error?.issues.some((issue) => issue.path.join(".") === "agentEstimate.probability")).toBe(
      true,
    );
  });
  ...
  it("rejects invalid enum values", () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error?.issues.some((issue) => issue.path.join(".") === "agentEstimate.confidence")).toBe(
      true,
    );
  });
  ...
  it("rejects deltas that do not match agent estimate minus Kalshi probability", () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error?.issues.some((issue) => issue.path.join(".") === "delta.probabilityPoints")).toBe(
      true,
    );
  });
});
