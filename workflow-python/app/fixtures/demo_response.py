...
DEMO_WORKFLOW_RESPONSE: dict[str, object] = {
    "schemaVersion": "1.0",
    ...
    "agentEstimate": {
        ...
        "confidence": "medium",
        "thesis": (
            "Recent indicators modestly favor the event resolving yes, but the signal is not strong enough for high "
            "confidence."
        ),
        "assumptions": [
    ...
    "delta": {"probabilityPoints": 0.13, "direction": "agent_higher"},
    "evidence": [
        {
    ...
    "settlementRisks": [
        {
            "risk": (
                "The intuitive event wording may differ from the exact threshold used by the official settlement "
                "source."
            ),
            "severity": "medium",
        }
    ...
    "warnings": [
        {
            "kind": "liquidity",
            "message": (
                "The spread is narrow enough for a demo but should still be shown as market context, not advice."
            ),
            "severity": "low",
        },
        {
    ...
    "finalMemoMarkdown": (
        "## Research Memo\n\nThe agent estimate is 55% versus Kalshi's 42% implied probability. The gap is "
        "driven by recent comparable indicators, but confidence remains medium because settlement wording and late "
        "data revisions could change the analysis."
    ),
...
def build_demo_response() -> WorkflowResponse:
    return WorkflowResponse.model_validate(DEMO_WORKFLOW_RESPONSE)
